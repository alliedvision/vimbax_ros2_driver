...
result<std::vector<std::string>> VimbaXCamera::features_list_get(void) const
{
  ...
  if (err != VmbErrorSuccess) {
    RCLCPP_ERROR(
      ...
      vimbax_camera::helper::vmb_error_to_string(err).data());
    return error{err};
  }
...
VimbaXCamera::feature_enum_as_string_get(const std::string_view & name, const int64_t value) const
{
  ...
  if (err != VmbErrorSuccess) {
    RCLCPP_ERROR(
      ...
      name.data(), option.data(), err, vimbax_camera::helper::vmb_error_to_string(err).data());
  ...
  for (auto name : names) {
    ...
    if (err != VmbErrorSuccess) {
      RCLCPP_ERROR(
        ...
        err, vimbax_camera::helper::vmb_error_to_string(err).data());
      return error{err};
    }
